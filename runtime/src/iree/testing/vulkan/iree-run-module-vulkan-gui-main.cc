...
iree_status_t GetModuleContentsFromFlags(iree_file_contents_t** out_contents) {
  ...
  if (module_file == "-") {
    fprintf(stderr, "[vulkan] Reading module contents from stdin \n");
...
extern "C" int iree_main(int argc, char** argv) {
  ...
  if (iree_status_is_ok(status)) {
    fprintf(stderr, "IREE runtime API version %u", actual_version);
  } else {
    fprintf(stderr, "Unsupported IREE runtime API version %u", actual_version);
  }
  ...
  fprintf(stderr, "Creating Vulkan driver/device");
  ...
  IREE_CHECK_OK(iree_hal_module_create(iree_instance, iree_vk_device,
                                       IREE_HAL_MODULE_FLAG_NONE, iree_allocator_system(),
                                       &hal_module));

  // Load bytecode module from embedded data.
  fprintf(stderr, "Loading IREE bytedcode module");
  ...
  fprintf(stderr, "Context with modules is ready for use");
  ...
  fprintf(stderr, "Resolved main function named '%s'",
      std::string(main_function_name.data, main_function_name.size).c_str());
