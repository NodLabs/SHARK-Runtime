...
FailureOr<OpIndex> getDefiningOpIndex(
  ...
  if (operand.get().dyn_cast<BlockArgument>()) {
    return 0;
  }
  ...
  if (operand.get().getDefiningOp()->getBlock() != &block) {
    operand.getOwner()->emitError(
        "Can't extract call graph for block that is not isolated from above.");
    return failure();
  }

  auto it = operationInBlockIndexMap.find(operand.get().getDefiningOp());
...
std::string getOpId(Operation& op) {
  ...
  func::ReturnOp returnOp = dyn_cast<func::ReturnOp>(op);
  if (returnOp) {
    return "return";
  }
