...
// Inserts flow.tensor.trace ops around the specified op.
template <typename DispatchOpTy>
static void traceOpWithName(DispatchOpTy dispatchOp, std::string name) {
...
static LogicalResult replaceReturnWithOpResults(mlir::ModuleOp moduleOp,
  ...
  if (op->getBlock()->getParentOp() != funcOp) return failure();

  auto funcBlock = op->getBlock();
  assert(funcBlock->getParentOp() == funcOp);
...
class InsertDebugTargetAtOrdinalPass
 ...
 public:
  InsertDebugTargetAtOrdinalPass(std::string breakStr, std::string traceStr)
      : breakDebugTarget(breakStr), traceDebugTarget(traceStr){};
  ...
  void runOnOperation() override {
    ...
    for (auto it :
      ...
      if (localTraceOrdinal > 0 && localTraceOrdinal < dispatchOps.size()) {
        ...
        traceOpWithName<DispatchOp>(traceTarget,
                                    entryPointName + std::string("::") +
 ...
 private:
  std::string breakDebugTarget;
  std::string traceDebugTarget;
};

// Insert break/tracing by symbol, after outlining dispatch regions and
// deduplication.
class InsertDebugTargetAtSymbolPass
  ...
  void runOnOperation() override {
    for (auto it :
      ...
      funcOp.walk([&](DispatchOp dispatchOp) {
        ...
        if (entryPointName.find(traceDebugTarget) != std::string::npos)
          traceOpWithName<DispatchOp>(dispatchOp, entryPointName);
      ...
      // no control flow within the function.
      if (breakTarget) {
